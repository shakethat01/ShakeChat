...
fn configure_processing(audio: &PlatformAudio, processing: NativeAudioProcessing) -> Result<(), String> {
    audio
        .configure_audio_processing(AudioProcessingOptions {
            ...
            // Windows/macOS/Linux use WebRTC's software APM. Keeping this false
            // makes the desktop path deterministic and avoids browser DSP entirely.
...
fn set_remote_audio_enabled(room: &Room, enabled: bool) {
    for participant in room.remote_participants().values() {
        for publication in participant.track_publications().values() {
            if !matches!(publication.source(), TrackSource::Microphone | TrackSource::ScreenshareAudio) {
                continue;
            }
...
async fn close_previous(inner: &mut Inner) -> Option<Room> {
    if let Some(task) = inner.room_events.take() {
        task.abort();
    }
...
pub async fn native_voice_join(
    ...
    let old_room = {
        ...
        close_previous(&mut inner).await
    };
    ...
    let input_device_id = audio
        .recording_devices()
        .next()
        .map(|device| device.id.to_string())
        .unwrap_or_default();
    let output_device_id = audio
        .playout_devices()
        .next()
        .map(|device| device.id.to_string())
        .unwrap_or_default();
    ...
    let event_task = tauri::async_runtime::spawn(async move {
        while events.recv().await.is_some() {
            // Draining RoomEvent keeps the SDK event channel healthy. UI state is
            // read directly from Room snapshots, so no browser-style event bridge
            // is needed for the native audio engine.
        }
    });
    ...
    let mic_track = if can_speak {
        ...
        let publish = TrackPublishOptions {
            ...
            // Keep low-energy speech tails flowing. Browser DTX was one of the
            // variables that made short syllables difficult to diagnose.
            dtx: false,
            red: true,
    ...
    inner.muted = start_muted || !can_speak;
    inner.deafened = false;
...
pub async fn native_voice_leave(state: State<'_, NativeVoiceState>) -> Result<(), String> {
    ...
    if let Some(room) = old_room {
        room.close()
            .await
            .map_err(|error| map_error("Ses odasından çıkılamadı", error))?;
    }
    Ok(())
}
...
pub async fn native_voice_set_muted(
    ...
    let mut inner = state.inner.lock().await;
    if !inner.can_speak && !muted {
        return Err("Bu ses kanalında konuşma yetkin yok.".into());
    }
...
#[tauri::command]
pub async fn native_voice_set_processing(
    state: State<'_, NativeVoiceState>,
    processing: NativeAudioProcessing,
) -> Result<(), String> {
    let mut inner = state.inner.lock().await;
...
#[tauri::command]
pub async fn native_voice_switch_input(
    state: State<'_, NativeVoiceState>,
    device_id: String,
) -> Result<(), String> {
    let mut inner = state.inner.lock().await;
    let Some(audio) = inner.audio.as_ref() else {
        return Err("Yerel ses motoru bağlı değil.".into());
    };
    let device = audio
        .recording_devices()
    ...
    audio
        ...
        .map_err(|error| map_error("Mikrofon değiştirilemedi", error))?;
    inner.input_device_id = device_id;
    Ok(())
}

#[tauri::command]
pub async fn native_voice_switch_output(
    state: State<'_, NativeVoiceState>,
    device_id: String,
) -> Result<(), String> {
    let mut inner = state.inner.lock().await;
    ...
    audio
        ...
        .map_err(|error| map_error("Hoparlör değiştirilemedi", error))?;
    inner.output_device_id = device_id;
    Ok(())
}

#[tauri::command]
pub async fn native_voice_set_participant_muted(
    state: State<'_, NativeVoiceState>,
    identity: String,
    muted: bool,
) -> Result<(), String> {
    ...
    let Some(room) = inner.room.as_ref() else {
        return Err("Ses odasına bağlı değilsin.".into());
    };
    let participant = room
        .remote_participants()
        .into_values()
        .find(|participant| participant.identity().to_string() == identity)
...
#[tauri::command]
pub async fn native_voice_snapshot(
    state: State<'_, NativeVoiceState>,
) -> Result<NativeVoiceSnapshot, String> {
    let inner = state.inner.lock().await;
    let Some(room) = inner.room.as_ref() else {
        return Ok(NativeVoiceSnapshot {
            ...
            input_device_id: inner.input_device_id.clone(),
            output_device_id: inner.output_device_id.clone(),
            engine: "native-webrtc-apm".into(),
        });
    };
    ...
    participants.push(NativeVoiceParticipant {
        identity: local_identity.clone(),
        name: if local_name.trim().is_empty() { local_identity } else { local_name },
        local: true,
    ...
    for participant in room.remote_participants().values() {
        ...
        let mic = publications
            .values()
            .find(|publication| publication.source() == TrackSource::Microphone);
        let camera = publications
            .values()
            .any(|publication| publication.source() == TrackSource::Camera && !publication.is_muted());
        let screen = publications
            .values()
            .any(|publication| publication.source() == TrackSource::Screenshare && !publication.is_muted());
        let identity = participant.identity().to_string();
        let name = participant.name();
        participants.push(NativeVoiceParticipant {
            identity: identity.clone(),
            name: if name.trim().is_empty() { identity } else { name },
            local: false,
