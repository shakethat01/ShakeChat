      ...
      <section className="app-settings-content">
        ...
        {tab==='account'?<div className="settings-stack">
          ...
          <div className="setting-block"><div className="setting-title"><AudioLines size={18}/><div><b>Mikrofon işleme</b><small>Tarayıcının WebRTC ses işleme seçenekleri.</small></div></div><div className="audio-toggle-list">
            <label className="setting-toggle"><span><b>Gürültü azaltma (RNNoise)</b><small>Konuşurken de arka plan seslerini azaltır. Ses kapısından bağımsız çalışır.</small></span><input type="checkbox" checked={draft.noiseSuppression} onChange={e=>setDraft(p=>({...p,noiseSuppression:e.target.checked}))}/><i/></label>
            ...
            <label className="setting-toggle"><span><b>Ses kapısı (Noise Gate)</b><small>Konuşmadığın anlarda klavye ve oda sesini yumuşak biçimde bastırır.</small></span><input type="checkbox" checked={draft.noiseGateEnabled} onChange={e=>setDraft(p=>({...p,noiseGateEnabled:e.target.checked}))}/><i/></label>
