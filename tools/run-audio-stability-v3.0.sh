...
echo "[1/10] Patch script syntax hotfix + node --check..."
node tools/fix-audio-stability-v3.1-syntax.mjs

echo
echo "[2/10] Ana stabilite patch'i uygulanıyor..."
node tools/apply-audio-stability-v3.0.mjs

echo
echo "[3/10] Duplicate runtime tuning hotfix..."
node tools/apply-audio-stability-v3.0-hotfix.mjs

echo
echo "[4/10] Worklet reuse v3.1..."
node tools/apply-audio-worklet-reuse-v3.1.mjs

echo
echo "[5/10] Profil restart hata bildirimi..."
node tools/apply-audio-stability-v3.1-error-surface.mjs

echo
echo "[6/10] Statik verify..."
node tools/verify-audio-stability-v3.0.mjs

echo
echo "[7/10] git diff --check..."
git diff --check

echo
echo "[8/10] TypeScript typecheck..."
npm run typecheck

echo
echo "[9/10] Production web build..."
VITE_API_ORIGIN=same-origin npm run build

echo
echo "[10/10] Tum testler..."
